...
/**
 * <code>{@link XMLIndexes}</code>
 *
 * TODO : document me
 *
 * @author Calvin Chen
 */
public class XMLIndexes extends SimpleList<XMLIndex>{

	private static final Logger log = LoggerFactory.getLogger(XMLIndexes.class);
	 ...
	 * @param xmlTable
	 */
	public XMLIndexes(Node currentNode, XMLTable xmlTable) {
		this.superTable = xmlTable;
		
		XMLIndex index = null;
		List<Node> nodeList = getNodes(O_INDEX, currentNode);
		if(ListUtil.isNotEmpty(nodeList)){
			for (Node node : nodeList) {
				index = new XMLIndex(node, this);
				add(index);
				IdCollector.put(index.getId(), index);
			}
			log.debug(INITIALLIZED_NAME, "XMLIndexes");
		}
	}
