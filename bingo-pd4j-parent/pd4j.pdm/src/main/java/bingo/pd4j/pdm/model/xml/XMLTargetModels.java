...
/**
 * <code>{@link XMLTargetModels}</code>
 *
 * TODO : document me
 *
 * @author Calvin Chen
 */
public class XMLTargetModels extends SimpleList<XMLTargetModel>{
	
	private static final Logger log = LoggerFactory.getLogger(XMLTargetModels.class);
	 ...
	 * @param xmlModel
	 */
	public XMLTargetModels(Node currentNode, XMLModel xmlModel) {
		this.superModel = xmlModel;
		
		XMLTargetModel targetModel = null;
		List<Node> nodeList = getNodes(O_TARGET_MODEL, currentNode);
		if(ListUtil.isNotEmpty(nodeList)){
			for (Node node : nodeList) {
				targetModel = new XMLTargetModel(node, this);
				add(targetModel);
				IdCollector.put(targetModel.getId(), targetModel);
			}
			log.debug(INITIALLIZED_NAME, "XMLTargetModels");
		}
	}
